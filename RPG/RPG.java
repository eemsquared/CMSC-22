    ...
    // returns true if someone is killed
     ...
     public boolean monsteroffense(RPGCharacter attacker, RPGCharacter defender, int choice) {
        ...
        if (coinToss()) { 
        	...
        	if (choice == 3) {
        		if(SRose == 1) {
        			System.out.println("\nMIC MIC ACTIVATED REFLECTION! \n");
                    sleep(1000);
                    System.out.println(attacker.getName() + " hurts himself\n");
                    sleep(2500);
                    monsterDamage = monster.attack();
                    remHp = attacker.takeDamage(monsterDamage);
                    if (remHp <= 0) {
                    	System.out.println("\n"+attacker.getName()+" destroyed himself");
 	                    sleep(1000);
 	                    return true;
                    }
                    return false;
